...
# 同じ位置にあるノーツを探す
def _search_samepos_note(note_info: tuple[int, int], notes: list, remove: bool) -> int | None:
    for note in notes[:]:
...
def _search_is_critical(tap_note: int | None) -> bool:
    if (
        tap_note == SusNoteType.Tap.C_TAP or
        tap_note == SusNoteType.Tap.C_TRACE or
        tap_note == SusNoteType.Tap.C_ELASER
    ):
        return True
    return False
...
def _search_is_trace(tap_note: int | None) -> bool:
    if (
        tap_note == SusNoteType.Tap.TRACE or
        tap_note == SusNoteType.Tap.C_TRACE
    ):
        return True
    return False
...
# フリックの向きを調べる
def _search_directional_type(air_note: int | None) -> Literal["left", "up", "right"] | None:
    match air_note:
...
def load(fp: TextIO) -> Score:
    ...
    for til in sorted(sus_score.tils, key=lambda x: x[0]):
        ...
        time_scale_group.append(
            TimeScalePoint(
                beat = _tick_to_beat(til[0]),
                timeScale = til[1]
            )
        )
    if not exist_initial_time_scale:
        time_scale_group.insert(
            0, 
            TimeScalePoint(
                beat = 0.0,
                timeScale = 1.0
    ...
    for slide in sus_score.slides:
        ...
        for idx, point in zip(range(point_length), sorted(slide, key=lambda x: x.tick)):
            samepos_tap = _search_samepos_note((point.tick, point.lane), sus_score.taps, remove=True)
            samepos_direction = _search_samepos_note((point.tick, point.lane), sus_score.directionals, remove=True)
    ...
    for note in sorted(sus_score.taps, key=lambda x: x.tick):
        samepos_direction = _search_samepos_note((note.tick, note.lane), sus_score.directionals, remove=True)
